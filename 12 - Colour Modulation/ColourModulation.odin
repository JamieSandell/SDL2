...
main :: proc() {
    ...
    for !quit {
        for sdl.PollEvent(&e) {
            if e.type == sdl.EventType.QUIT {
                ...
                #partial switch e.key.keysym.sym {
                    case sdl.Keycode.q : r = clamp(r + 32, r, 255)
                    case sdl.Keycode.w : g = clamp(g + 32, g, 255)
                    case sdl.Keycode.e : b = clamp(b + 32, b, 255)
                    case sdl.Keycode.a : r = clamp(r - 32, 0, r)
                    case sdl.Keycode.s : g = clamp(g - 32, 0, g)
                    case sdl.Keycode.d : b = clamp(b - 32, 0, b)
                }
            }
        }
        ...
        texture_set_colour(global_texture, r, g, b)
        texture_render(global_texture, 0, 0, nil)
...
load_media :: proc() -> bool {
    if !texture_load_from_file(&global_texture, "./assets/colors.png") {
        fmt.eprintln("Failed to load dots colors image.")
        return false
    }
